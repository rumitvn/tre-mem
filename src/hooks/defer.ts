...
export interface DeferInput {
  format: HookFormat;
  /** claude-mem is present on disk (its banner will print on Claude Code). */
 ...
 * always wins (0 disables; blank/malformed is ignored). Otherwise only Claude
 * Code with claude-mem present defers at all, and the first session of a project
 * (claude-mem's slower onboarding banner) defers longer than steady state.
 */
