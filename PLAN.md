...
> Tre-mem là tầng memory branch-aware, local-first, build _trên_ claude-mem để AI hiểu đúng _feature đang làm_, không chỉ _repo đang ở_.
...
> [`PLAN-PHASE6.md`](./PLAN-PHASE6.md) — Phase 6 (v0.8.x, "The Grove" — contributor graph). Current: **v0.8.0**.
...
async function getBranchContext(
  query: string,
  opts: { project: string; branch?: string; k?: number },
) {
...
Với mỗi observation cũ: lấy `cwd`, `created_at_epoch` từ pending*messages join sdk_sessions → parse reflog → tìm transition gần nhất \_trước* epoch → branch → insert `branch_tag` source='reflog-backfill'.
