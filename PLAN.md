...
- [ ] **T2D9** E2E: trong repo có ≥2 branch memory → checkout từng branch → ask Claude → assert khác nhau & đúng feature — partial: hook reports correct branch (`feature/test_tre_mem` ↔ `feature/test_tre_mem_2`), `tagged_on_branch=0` until `tre backfill` is run against multigo-android-dev
