...
- [ ] **T2D9** Đăng ký `mcpServers.tre-mem` vào `~/.claude.json` — script ready (`node dist/cli.js mcp`), pending explicit user consent (Claude Code auto-mode blocked self-modification of `~/.claude.json`)
- [ ] **T2D9** E2E: trong repo có ≥2 branch memory → checkout từng branch → ask Claude → assert khác nhau & đúng feature — blocked on multi-branch dataset (current `tre-mem.db` only has `main`)
